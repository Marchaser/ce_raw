...
gen CASHCO = 1 if 0  | inlist(UCC,800111,800121,800804,800811,800821,800831,800841,800851,800861);
replace CASHCO = 0 if CASHCO==.;
gen MISC2 = 1 if 0  | inlist(UCC,710110);
...
gen EDUCA = 1 if 0  | inlist(UCC,660110,660210,660410,660901,660902,670110,670210,670410,670901,670902,670903);
replace EDUCA = 0 if EDUCA==.;
gen READ = 1 if 0  | inlist(UCC,590220,590230,590310,590410,660310);
replace READ = 0 if READ==.;
gen PERSCA = 1 if 0  | inlist(UCC,640130,640420,650310);
replace PERSCA = 0 if PERSCA==.;
gen OTHENT = 1 if 0  | inlist(UCC,520901,520904,520907,600110,600121,600122,600132,600141,600142,600210,600310,600410,600420,600430,600901,600902,610210,610230,620320,620330,620905,620906,620908,620909,620919,620921,620922,620930,680310,680320);
replace OTHENT = 0 if OTHENT==.;
gen PETTOY = 1 if 0  | inlist(UCC,610110,610120,610320,620410,620420,610140);
replace PETTOY = 0 if PETTOY==.;
gen OTHEQP = 1 if 0  | PETTOY  |  OTHENT;
replace OTHEQP = 0 if OTHEQP==.;
gen TVRDIO = 1 if 0  | inlist(UCC,270310,270311,310140,310210,310220,310240,310311,310313,310314,310320,310333,310334,310340,310350,340610,340902,340905,610130,690320,690330,690340,690350,620904,620912);
...
gen MEDSRV = 1 if 0  | inlist(UCC,560110,560210,560310,560330,560400,570111,570220,570230,570240);
replace MEDSRV = 0 if MEDSRV==.;
gen HLTHIN = 1 if 0  | inlist(UCC,580111,580112,580113,580114,580311,580312,580400,580901,580903,580904,580905,580906,580907);
...
gen VRNTLO = 1 if 0  | inlist(UCC,450310,450313,450314,450410,450413,450414,520110,520310,520410,520511,520512,520521,520522,520531,520532,520541,520542,520550,520560,520902,520905,620113);
replace VRNTLO = 0 if VRNTLO==.;
gen VEHINS = 1 if 0  | inlist(UCC,500110);
replace VEHINS = 0 if VEHINS==.;
gen MAINRP = 1 if 0  | inlist(UCC,470220,480110,480212,480213,480214,480215,490110,490211,490212,490221,490231,490232,490311,490312,490313,490314,490318,490319,490411,490412,490413,490501,490900);
...
gen TRANS = 1 if 0  | CARTKN  |  CARTKU  |  OTHVEH  |  GASMO  | 
VEHFIN  |  MAINRP  |  VEHINS  |  VRNTLO  | 
PUBTRA;
...
gen GRLFIF = 1 if 0  | inlist(UCC,390110,390120,390210,390221,390222,390223,390230,390310,390321,390322,390901,390902);
replace GRLFIF = 0 if GRLFIF==.;
gen WOMSIX = 1 if 0  | inlist(UCC,380110,380210,380311,380312,380313,380320,380331,380332,380333,380340,380410,380420,380430,380510,380901,380902,380903);
replace WOMSIX = 0 if WOMSIX==.;
gen WOMGRL = 1 if 0  | WOMSIX  |  GRLFIF;
replace WOMGRL = 0 if WOMGRL==.;
gen BOYFIF = 1 if 0  | inlist(UCC,370110,370120,370130,370211,370212,370213,370220,370311,370312,370313,370314,370902,370903,370904);
replace BOYFIF = 0 if BOYFIF==.;
gen MENSIX = 1 if 0  | inlist(UCC,360110,360120,360210,360311,360312,360320,360330,360340,360350,360410,360511,360512,360513,360901,360902);
replace MENSIX = 0 if MENSIX==.;
gen MENBOY = 1 if 0  | MENSIX  |  BOYFIF;
replace MENBOY = 0 if MENBOY==.;
gen APPAR = 1 if 0  | MENBOY  |  WOMGRL  |  CHLDRN  |  FOOTWR
 |  OTHAPL;
replace APPAR = 0 if APPAR==.;
gen MISCEQ = 1 if 0  | inlist(UCC,320120,320130,320150,320220,320232,320233,320410,320420,320901,320902,320903,320904,340904,430130,690111,690115,690210,690230,690241,690242,690243,690244,690245,690117);
...
gen FLRCVR = 1 if 0  | inlist(UCC,320111);
...
gen HOUSEQ = 1 if 0  | TEXTIL  |  FURNTR  |  FLRCVR  |  MAJAPP  | 
SMLAPP  |  MISCEQ;
replace HOUSEQ = 0 if HOUSEQ==.;
gen OTHHEX = 1 if 0  | inlist(UCC,330511,340510,340620,340630,340901,340907,340908,340915,690113,690114,690116,690310,990900);
replace OTHHEX = 0 if OTHHEX==.;
gen BBYDAY = 1 if 0  | inlist(UCC,340211,340212,670310);
replace BBYDAY = 0 if BBYDAY==.;
gen DMSXCC = 1 if 0  | inlist(UCC,340310,340410,340420,340520,340530,340903,340906,340910,340914);
...
gen TELEPH = 1 if 0  | inlist(UCC,270101,270102,270104,270105);
replace TELEPH = 0 if TELEPH==.;
gen OTHFLS = 1 if 0  | inlist(UCC,250211,250212,250213,250214,250911,250912,250913,250914);
...
gen OTHLOD = 1 if 0  | inlist(UCC,210210,210310,210902,220122,220212,220312,220314,220902,230123,230152,230902,240113,240123,240214,240223,240313,240323,320613,320623,320633,340912,880310,990940,680905);
replace OTHLOD = 0 if OTHLOD==.;
gen RNTAPY = 1 if 0  | inlist(UCC,800710);
replace RNTAPY = 0 if RNTAPY==.;
gen RNTXRP = 1 if 0  | inlist(UCC,210110,230121,230141,230150,240111,240121,240211,240221,240311,240321,320611,320621,320631,350110,790690,990920);
replace RNTXRP = 0 if RNTXRP==.;
gen RENDWE = 1 if 0  | RNTXRP  |  RNTAPY;
replace RENDWE = 0 if RENDWE==.;
gen MRPINS = 1 if 0  | inlist(UCC,210901,220121,220901,230112,230113,230114,230115,230122,230142,230151,230901,240112,240122,240212,240213,240222,240312,240322,320612,320622,320632,340911,990930);
replace MRPINS = 0 if MRPINS==.;
gen PROPTX = 1 if 0  | inlist(UCC,220211);
replace PROPTX = 0 if PROPTX==.;
gen MRTINT = 1 if 0  | inlist(UCC,220311,220313,220321,880110);
...
gen ALCBEV = 1 if 0  | inlist(UCC,200900,790310,790320,790330,790420);
...
gen FDHOME = 1 if 0  | inlist(UCC,190904,790220,790230,790240);
replace FDHOME = 0 if FDHOME==.;
gen FOOD = 1 if 0  | FDHOME  |  FDAWAY;
replace FOOD = 0 if FOOD==.;
gen TOTEXP = 1 if 0  | FOOD  |  ALCBEV  |  HOUS  |  APPAR  | 
TRANS  |  HEALTH  |  ENTERT  |  PERSCA  | 
READ  |  EDUCA  |  TOBACC  |  MISC  | 
CASHCO  |  PERINS;
