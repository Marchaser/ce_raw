...
// 1984 in stata
local i=1
foreach subfile in 0003 0007 0011 0015 0019{
	use ${SRC}\1984\08671-`subfile'-Data, clear
	destring UCC, replace
	save e\e1984q`i',replace
	local i=`i'+1
}
...
forval iYear=1/`numYear' {
	...
	foreach subfile in `year'1x `year'2 `year'3 `year'4 `nextYear'1{
		...
		if `year'<90 {
			save e\e20`year'q`i', replace
		}
		else {
			save e\e19`year'q`i', replace
		}
		local i=`i'+1
	}
}

// can't append all due to large data size
/*
// append all
clear
forval year=1984/2013{
forval quarter=1/5{
	cap append using e`year'q`quarter'
}
}
keep NEWID UCC COST REF_MO REF_YR
