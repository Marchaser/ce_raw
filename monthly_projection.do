...
foreach v in INDPRO TOTEXP1 TOTEXP2 rPCE CPIAUCSL PPIACO{
	gen l`v'=log(`v')
}
...
gen res0se=.
forval t=0/48 {
	reg f`t'.lTOTEXP2 resid L(1/2).lTOTEXP2 L(1/2).resid L(1/2).FEDFUNDS L(1/2).lCPIAUCSL L(1/2).UNRATE L(1/2).PPIACO
...
gen res0se=.
forval t=0/48 {
	reg f`t'.lTOTEXP1 resid L(1/2).lTOTEXP1 L(1/2).resid L(1/2).FEDFUNDS L(1/2).lCPIAUCSL L(1/2).UNRATE L(0/2).PPIACO
	replace res0=_b[resi] if _n==`t'
	replace res0se=_se[resi] if _n==`t'
}
tsline res0 if res0~=.
